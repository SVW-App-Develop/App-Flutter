...
🚨 핵심 요약
---
- **Class** 키워드를 사용해서 클래서 선언 가능

- 클래스를 **인스턴스화**하면 클래스의 인스턴스를 변수로 저장 가능

- **상속**받으면 부모 클래스의 모든 속성을 물려받음

  - extends 키워드를 사용해서 상속받을 수 있음
 
  - 하나의 자식 클래스는 하나의 부모 클래스만 상속받을 수 있음
 
- **오버라이드** : 이미 선언되어 있는 속성을 덮어쓰는 기능

- **인터페이스** : 클래스의 필수 속성들을 정의하고 강제할 수 있는 기능

  - implements 키워드를 사용해서 인터페이스 적용
 
  - 하나의 클래스에 여러 개의 인터페이스 적용 가능
 
- **믹스인** : 상속처럼 모든 속성을 물려받지 않고 원하는 기능만 골라서 적용 가능

  - with 키워드를 사용해 믹스인 적용
 
  - 하나의 클래스에 여러 개의 믹스인 적용 가능
 
- **제네릭** : 변수 타입의 정의를 인스턴스화까지 미룰 수 있음

  - <> 를 사용해서 제네릭 선언 가능
 
- **스태틱** : 클래스에 직접 귀속되는 속성들

  - static 키워드를 사용해서 선언
 
- **캐스케이드 연산자** : 인스턴스에서 해당 인스턴스 속성이나 멤버 함수를 연속해서 호출할 때 사용
